...
if [[ "${UPDATE_ENVIRONMENT}" == "Y" ]]; then
  ...
  if [[ "${DEPLOY_SYSTEMD}" == "Y" ]]; then
    ...
    if grep -q "ENABLE_METRICS_SERVER=true" ${CNODE_HOME}/mithril/mithril.env; then
      ...
      echo "PARTY_ID=${PARTY_ID} AGGREGATOR_ENDPOINT=${AGGREGATOR_ENDPOINT} ${MITHRILBIN} ${METRICS_SERVER_PARAMS} -vvvvv "
      if ! "PARTY_ID=${PARTY_ID}" "AGGREGATOR_ENDPOINT=${AGGREGATOR_ENDPOINT}" "${MITHRILBIN}" "${METRICS_SERVER_PARAMS}" -vvvvv | tee -a "${LOG_DIR}/$(basename "${0::-3}")".log 2>&1 ; then
        echo "Failed to start Mithril Signer Server with metrics enabled" | tee -a "${LOG_DIR}/$(basename "${0::-3}")".log 2>&1
        exit 1
      fi
    else
      if ! "PARTY_ID=${PARTY_ID}" "AGGREGATOR_ENDPOINT=${AGGREGATOR_ENDPOINT}" "${MITHRILBIN}" -vvvvv | tee -a "${LOG_DIR}/$(basename "${0::-3}")".log 2>&1 ; then
